...
class FirestorePlanningStore(PlanningStore):
    ...
    async def create_session(self, session: PlanningSession) -> None:
        ...
        await asyncio.to_thread(ref.set, _to_document_payload(session))
    ...
    async def append_event(self, session_id: str, event: PlanningEvent) -> None:
        ...
        doc_ref = self._events_ref(session_id).document(event.id)
        await asyncio.to_thread(doc_ref.set, _to_document_payload(event))
    ...
    async def add_artifact(self, session_id: str, artifact: PlanningArtifact) -> None:
        ...
        doc_ref = self._artifacts_ref(session_id).document(artifact.id)
        await asyncio.to_thread(doc_ref.set, _to_document_payload(artifact))
    ...
    async def get_events(
        ...
        docs = await asyncio.to_thread(
            lambda: list(self._events_ref(session_id).order_by("created_at").stream())
        )
        events = [_event_from_payload(doc.to_dict() or {}, doc.id) for doc in docs]
    ...
    async def get_artifacts(self, session_id: str) -> list[PlanningArtifact]:
        ...
        docs = await asyncio.to_thread(
            lambda: list(self._artifacts_ref(session_id).order_by("created_at").stream())
        )
        return [_artifact_from_payload(doc.to_dict() or {}, doc.id) for doc in docs]
    ...
    async def sweep_expired_sessions(
        ...
        *,
        ...
        changed = 0
        intake_cutoff = now - timedelta(hours=intake_ttl_hours)
        running_cutoff = now - timedelta(hours=running_ttl_hours)
        collection = self._db.collection("planning_sessions")

        intake_docs = await asyncio.to_thread(
            lambda: list(
                collection.where("status", "==", PLANNING_STATUS_INTAKE_PENDING)
                .where("created_at", "<", intake_cutoff.isoformat())
                .stream()
            )
        )
        ...
        for doc in intake_docs:
            ...
            event = _build_event(session.id, "session_expired", {"status": PLANNING_STATUS_EXPIRED})
            doc_ref = self._events_ref(session.id).document(event.id)
            await asyncio.to_thread(doc_ref.set, _to_document_payload(event))
            changed += 1
        ...
        for doc in running_docs:
            ...
            event = _build_event(
                session.id, "session_timed_out", {"status": PLANNING_STATUS_TIMED_OUT}
            )
            doc_ref = self._events_ref(session.id).document(event.id)
            await asyncio.to_thread(doc_ref.set, _to_document_payload(event))
            changed += 1
...
def _to_document_payload(
    model: (PlanningSession | PlanningMessage | PlanningEvent | PlanningArtifact),
) -> dict[str, Any]:
