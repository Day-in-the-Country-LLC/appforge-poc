...
- Fifth, the agent creating the issues should use a label for each issue to specify the level of difficulty required to complete the task.  There are three difficulty labels: 'difficulty:easy', 'difficulty:medium', 'difficulty:hard'. These will map to the coding model used to complete the task: claude-haiku-4.5, claude-sonnet-4.5 or claude-opus-4.5 respectively.  Always ensure the appropriate difficulty label is specified in every issue.  
